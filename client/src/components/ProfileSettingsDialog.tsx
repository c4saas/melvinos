...
function ConnectedAccountsCard() {
  ...
  const disconnectMutation = useMutation({
    mutationFn: async (label: string) => {
      await apiRequest('DELETE', `/api/google-drive/disconnect?label=${encodeURIComponent(label)}`);
    },
  ...
  const connectWithLabel = (label?: string) => {
    const url = label ? `/auth/google?label=${encodeURIComponent(label)}` : '/auth/google';
    ...
    <Card>
      <CardHeader>
        ...
        <CardDescription className="text-xs sm:text-sm">
          Link Google accounts to enable Calendar, Gmail, and Drive access
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-2">
        {isLoading ? (
          ...
          <>
            {accounts.map((acct) => (
              <div key={acct.label} className="flex items-center justify-between gap-3 rounded-lg border border-border/50 bg-muted/30 p-3 sm:p-4">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-[10px] sm:text-xs text-muted-foreground truncate">
                      {acct.label !== 'default' ? acct.label : 'Default account'}
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  <Button
                    ...
                    onClick={() => disconnectMutation.mutate(acct.label)}
            ...
            {accounts.length === 0 && (
              <div className="flex items-center justify-between gap-3 rounded-lg border border-border/50 bg-muted/30 p-3 sm:p-4">
                ...
                <Button variant="outline" size="sm" className="text-xs h-7 flex-shrink-0" onClick={() => connectWithLabel()}>
                  Connect
                </Button>
              </div>
            )}
            ...
            {accounts.length > 0 && (
              <Button
                ...
                onClick={() => {
                  const label = prompt('Account label (e.g. "Work", "Agency", "Personal"):');
                  if (label?.trim()) connectWithLabel(label.trim());
                }}
              >

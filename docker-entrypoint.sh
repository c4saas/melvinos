...
# ── Drop to atlas user and exec the app ─────────────────────────────────────
exec su -s /bin/sh -c 'exec "$@"' atlas atlas "$@"
